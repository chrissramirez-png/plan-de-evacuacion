...
/* ─── API helpers ────────────────────────────────────────────── */
const callGemini = async ({ max_tokens, messages }) => {
...
async function analyzeFloorPlan(imageDataUrl) {
...
async function analyzeEmergencyPlanPDF(pdfDataUrl) {
...
async function fetchAISuggestion({ elements, routes, buildingCtx, userLocation, emergencyType, floors }) {
  const fmt    = (arr) => arr.map((e) => `"${e.label}" en (${e.x.toFixed(0)}%,${e.y.toFixed(0)}%)`);
...
const GLOBAL_CSS = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  @keyframes fe      { to { stroke-dashoffset: -16; } }
  @keyframes slideIn { from { opacity:0; transform:translateY(-6px); } to { opacity:1; transform:translateY(0); } }

  .spin     { animation: spin 1s linear infinite; }
  .fl       { animation: fd  1.2s linear infinite; }
  .fem      { animation: fe  0.5s linear infinite; }
  ...
  .icon-btn:hover { background: #EEF3FE; color: #4E526E; }

  .card {
    background: #FFFFFF;
    border-radius: 16px;
    box-shadow: 0 2px 12px rgba(78,82,110,0.08);
    border: 1px solid #E8EBF0;
  }

  .input-field {
    background: #FFFFFF; border: 1.5px solid #E8EBF0; border-radius: 10px;
    padding: 8px 12px; font-size: 13px; font-family: 'Montserrat', system-ui, sans-serif;
    color: #4E526E; outline: none; width: 100%; box-sizing: border-box; transition: border-color 0.15s;
  }
  .input-field:focus { border-color: #4CBF8C; }

  .textarea-field {
    background: #FFFFFF; border: 1.5px solid #E8EBF0; border-radius: 10px;
    padding: 8px 12px; font-size: 12px; font-family: 'Montserrat', system-ui, sans-serif;
    color: #4E526E; outline: none; resize: none; width: 100%; box-sizing: border-box;
    transition: border-color 0.15s;
  }
  .textarea-field:focus { border-color: #4CBF8C; }

  .btn-primary {
    background: #4CBF8C; color: white; border: none; border-radius: 12px;
    padding: 10px 20px; font-size: 13px; font-weight: 600;
    font-family: 'Montserrat', system-ui, sans-serif; cursor: pointer;
    display: flex; align-items: center; justify-content: center; gap: 8px;
    transition: background 0.15s;
  }
  .btn-primary:hover:not(:disabled) { background: #3AA87A; }
  .btn-primary:disabled { opacity: 0.5; cursor: not-allowed; }

  .btn-outline {
    background: transparent; color: #4E526E; border: 1.5px solid #E8EBF0;
    border-radius: 12px; padding: 10px 20px; font-size: 13px; font-weight: 600;
    font-family: 'Montserrat', system-ui, sans-serif; cursor: pointer;
    display: flex; align-items: center; justify-content: center; gap: 8px;
    transition: all 0.15s;
  }
  .btn-outline:hover { border-color: #4CBF8C; color: #4CBF8C; }
  ...
  .element-pin:hover { transform: scale(1.2); }

  .tooltip {
    position: absolute; bottom: 42px; left: 50%; transform: translateX(-50%);
    background: white; border: 1px solid #E8EBF0; color: #4E526E;
    font-size: 11px; border-radius: 12px; padding: 10px 14px;
    white-space: nowrap; box-shadow: 0 4px 20px rgba(78,82,110,0.14);
    z-index: 30; pointer-events: none; font-family: 'Montserrat', system-ui, sans-serif;
  }

  .sidebar-section { padding: 12px 16px; border-bottom: 1px solid #F0F2F6; }
  ...
  .element-row {
    ...
    border-radius: 10px; background: #F8FAFC; border: 1px solid #F0F2F6;
    margin-bottom: 4px; font-size: 12px;
  }

  .suggestion-card {
    background: white; border: 1.5px solid #E8EBF0; border-radius: 12px;
    padding: 12px; margin-bottom: 8px; display: flex; align-items: flex-start; gap: 10px;
    animation: slideIn 0.2s ease-out; transition: border-color 0.15s;
  }
  .suggestion-card:hover { border-color: #4CBF8C; }
`;

/* ─── Custom Hooks ───────────────────────────────────────────── */
function useAI({ plan, userLocation, emergencyType }) {
...
function useMapEditor() {
  ...
  const handleCanvasClick = useCallback((e, { isViewMode, image }) => {
  ...
  // Used to place elements from AI suggestions
  const addElement = useCallback((type, { x, y, label, detail = "" }) => {
    setElements((prev) => [
      ...prev,
  ...
  // Used to add routes from AI suggestions
  const addRoute = useCallback((points) => {
  ...
  const removeElement = useCallback((id) => setElements((prev) => prev.filter((e) => e.id !== id)), []);
  const removeRoute   = useCallback((id) => setRoutes((prev) => prev.filter((r) => r.id !== id)), []);
...
function ApiKeyModal({ onClose }) {
  const [value, setValue] = useState(() => localStorage.getItem("anthropic_api_key") || "");
    ...
    >
      <div className="card" style={{ width: "100%", maxWidth: 520, maxHeight: "90vh", display: "flex", flexDirection: "column" }} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
          ...
          ].filter(({ items }) => items?.length > 0).map(({ label, items, icon }) => (
...
/* ════ RESIDENT VIEW ════════════════════════════════════════════ */
function ResidentView({ plan, onBack }) {
...
function EntryScreen({ onAdmin, onResident }) {
  ...
  const handleJoin = useCallback(async () => {
    ...
    try {
      ...
      if (!res) { setErr("Código no encontrado."); return; }
      onResident(JSON.parse(res.value), c);
    } catch {
  ...
  const handleRecover = useCallback(async () => {
    ...
    setSavedPlans(null);
    try {
      const res  = await window.storage.list("plan:", true);
      const keys = res?.keys || [];
      ...
      setSavedPlans(plans.filter(Boolean));
    } catch {
      setSavedPlans([]);
    } finally {
      setRecovering(false);
...
function AdminEditor({ onBack }) {
  ...
  const [locationFields, setLocationFields] = useState(DEFAULT_LOCATION_FIELDS);

  // AI floor plan analysis (image)
  ...
  const [pdfExtracted, setPdfExtracted]       = useState(null);
  ...
  const renameField = useCallback((id, val) => {
  ...
  const handlePdfFile = useCallback((e) => {
    const file = e.target.files[0];
    if (!file) return;
    setPdfError(null);
    ...
    reader.onload = async (ev) => {
      try {
        const result = await analyzeEmergencyPlanPDF(ev.target.result);
  ...
  const handleApplyPdf = useCallback(() => {
    ...
    // Reset and add elements
    reset();
    const addEl = (type, items) => {
    ...
    setShowCtx(true); // open building config so admin can review
  ...
  const handleAcceptSuggestion = useCallback((type, coords) => {
    addElement(type, coords);
  }, [addElement]);

  const handleAcceptRoute = useCallback((points) => {
    addRoute(points);
  }, [addRoute]);

  const publishPlan = useCallback(async (code) => {
    setSaving(true);
    ...
    try {
      await window.storage.set(`plan:${code}`, data, true);
      setShareCode(code);
      setShowShare(true);
    } catch (e) {
  ...
  return (
    <div className="app-root" style={{ display: "flex" }}>
      ...
      <nav style={{
        ...
        {/* Configuración del edificio (formerly "Contexto para IA") */}
        <div className="sidebar-section">
          <button
...
<p style={{ textAlign: "center", marginTop: 16, fontSize: 11, color: C.grayMid }}>
          ...
          <>
            ...
            <div
              ...
              onDoubleClick={() => activeTool === "ruta" && currentRoute?.length >= 2 && finishRoute()}
            >
              <img
                src={image} alt="Plano"
...
export default function App() {
  ...
  const [residentPlan, setResidentPlan] = useState(null);

  return (
    <>
      <style>{GLOBAL_CSS}</style>
      {screen === "admin"    && <AdminEditor onBack={() => setScreen("entry")} />}
      {screen === "resident" && <ResidentView plan={residentPlan} onBack={() => setScreen("entry")} />}
      {screen === "entry"    && (
        <EntryScreen
