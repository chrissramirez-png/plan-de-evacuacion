...
function PublicLoginContent() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(({ data: { session } }) => {
      if (session) {
        // Use identity data for domain check — avoids direct PII field access
  ...
  const handleLogin = async () => {
    ...
    // Guardar flow en sessionStorage para que el callback sepa redirigir
    sessionStorage.setItem("auth_flow", "public");

    const supabase = createClient();
    const { data, error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: {
        redirectTo: `${window.location.origin}/auth/callback?flow=public`,
